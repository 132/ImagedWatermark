function [Am2_Duong1 Duong2_Am1 Histogram] = bin(c,r,uh,u, watermark)
...
for T2am = 1:length(Threshold_Am(1,:))
    for T1duong = viTriBin0:length(Histogram(1,:))
        if size_W <= Threshold_Am(3,T2am) + Histogram(1,T1duong) && Histogram(1,T1duong) ~= 0 && Histogram(2,T1duong) ~= Histogram(2,Threshold_Am(1,T2am)) && Histogram(2,T1duong) ~= Histogram(2,Threshold_Am(2,T2am))
            viTriAm_1 = [viTriAm_1, Histogram(2,Threshold_Am(1,T2am))];
            viTriAm_2 = [viTriAm_2, Histogram(2,Threshold_Am(2,T2am))];
            viTriDuong_1 = [viTriDuong_1, Histogram(2,T1duong)];
            Tong_Am2_Duong1 = [Tong_Am2_Duong1, Threshold_Am(3,T2am) + Histogram(1,T1duong)];
        end
    end
end
Am2_Duong1 = [viTriAm_1; viTriAm_2; viTriDuong_1; Tong_Am2_Duong1];
...
for T2duong = 1:length(Threshold_Duong(1,:))
    for T1am = viTriBin0:-1:1
        if size_W <= Threshold_Duong(3,T2duong) + Histogram(1,T1am) && Histogram(1,T1am) ~= 0 && Histogram(2,T1am) ~= Histogram(2,Threshold_Duong(1,T2duong)) && Histogram(2,T1am) ~= Histogram(2,Threshold_Duong(2,T2duong))
            ...
            viTriAm_1 = [viTriAm_1, Histogram(2,T1am)];
            Tong_Duong2_Am1 = [Tong_Duong2_Am1, Threshold_Duong(3,T2duong) + Histogram(1,T1am)];
        end
    end
end
Duong2_Am1 = [viTriAm_1; viTriDuong_2; viTriDuong_1; Tong_Duong2_Am1];
