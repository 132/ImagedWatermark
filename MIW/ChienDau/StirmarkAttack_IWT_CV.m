...
for i = 1:length(TapCover)
    ...
    [c_Cross,r_Cross,uh_Cross,u_Cross] = crossset(CV);
    [CV_Modi size_W KEY Check] = embedding_Shitf0_dchiam_IWT_(Denta, m, CV,watermark,c_Cross,r_Cross,uh_Cross,u_Cross);
    watermarkedImage = ilwt2(CA,CH,CV_Modi,CD,LS);
    ...
    imwrite(watermarkedImage_Uint8,strcat('WI_IWT_CV\',TapCover(i).name));
    ...
    % extracting
        ...
        LS = liftwave('cdf2.2','Int2Int');
        [CA,CH,CV_Modi_,CD] = lwt2(double(watermarkedImage_Dung),LS);

        [c_Cross,r_Cross,uh_Cross,u_Cross] = crossset(CV_Modi_);
        [CV_OI, watermark] = extracting_Shift0_dchiam_IWT_(Denta, m, Check, CV_Modi_, size_W,KEY,c_Cross,r_Cross,uh_Cross,u_Cross);

        OImage = ilwt2(CA,CH,CV_OI,CD,LS);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    fid = fopen(strcat(TapCover(i).name,'_IWT_CV.txt')','w');
...
movefile('C:\Users\Administrator\Desktop\MyWork\stirmark\Media\Output\Images\Set1',strcat(TapCover(i).name,'_IWT_CV'));
...
delete(strcat('C:\Users\Administrator\Desktop\MyWork\stirmark\Media\Input\Images\Set1\',TapCover(i).name));
    ...
    src = dir(strcat(TapCover(i).name,'_IWT_CV\*.bmp'));
    for j = 1:length(src)
    filename = strcat(TapCover(i).name,'_IWT_CV\',src(j).name);
    I = imread(filename);
    try
    % extracted
